...
function showPostGame(winCreditName, displayName) {
    ...
    winnerBanner.textContent = displayName
...
async function fetchAllPokemon() {
    if (!amSpectator) statusBanner.innerText = "LOADING POKÉDEX...";
    try {
        ...
        if (!amSpectator) statusBanner.innerText = "FAILED TO LOAD — CHECK CONNECTION";
        console.error(err);
    }
}
...
function loop() {
    ...
    if (gameState==="LOADING") { statusBanner.innerText="LOADING POKÉDEX..."; }
        ...
        fighters.forEach(f=>{
            if(f.hp<=0)return; f.x+=f.vx; f.y+=f.vy;
...
function startBattle() {
    if (!playerChoice||amSpectator) return;
    selectionTimeLeft=0;
    selectionOverlay.style.display='none';
    countdownTimer=180;
    gameState="COUNTDOWN";
